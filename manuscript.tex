...
\Plaintitle{Interactively Visualizing Multivariate Market Segmentation Using the R Package Lionfish} %% without formatting
...
When there are more than two features, histograms of the individual features are still commonly used to display the partitioning results. This means that the analyst likely cannot understand how the partitioning divides the data. All that they can observe is roughly how the individual features relate to the partition, which is useful but inadequate. To understand how the partitions are formed in the data we need to view the linear combinations of variables, and often, focus on a small number of groups. Figure \ref{kmeans-linear-combinations} illustrates this approach, for the 2D, where it's not necessary, but will be useful to obtain a mental model of how this operates in higher dimensions. The partitions are cuts of the data, and so best viewed as dotplots. Plot A shows data A along V1, where the green and blue partitions are distinct (the orange and red partitions are not shown). Compare this with the full data plot in Figure \ref{kmeans-partition}. The red and orange partitions could be seen to be distinct if we plotted V2, and subsetted the data to these two groups. In plot B, the linear combination  that is roughly a contrast of V1 and V2 shows the distinction between the blue and green partitions of data B, and in plot C all four partitions can be seen when a linear combination of an equal combination of V1 and V2 is used. 
