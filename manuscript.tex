...
Market segmentation typically partitions data that does not have any clear cluster structure, resulting in groups of consumers who are similar, but without clear separation. The interpretation of such a clustering solution is typically obscured by the multivariate nature of the consumer data. The \texttt{lionfish} \proglang{R} package facilitates an interactive and dynamic exploration of clustering solutions. Unlike static approaches, \texttt{lionfish} allows users to refine clusters iteratively, focusing on aspects of the data most relevant to their specific analytical objectives. The package integrates tour algorithms through the \texttt{tourr} package, enabling multivariate visualization of high-dimensional data, while \proglang{Python}-powered interactivity allows for manual adjustments and redefinition of cluster boundaries based on visual feedback.
We analyzed two datasets derived from surveys conducted in Austria and Australia, where tourists reported on their vacation activities and show how we can understand and enhance a clustering solution using \texttt{lionfish} in those examples. The flexible, user-driven approach provides deeper insights into the complex market behaviours, facilitating more effective segmentation and improving strategic decision-making.
