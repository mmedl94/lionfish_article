...
There are several ways that these values can be normalized to examine different aspects: $f_{ij}^{o} = \frac{c_{ij}}{n}$, $f_{ij}^{c} = \frac{c_{ij}}{n_{i}}$ and $f_{ij}^{f} = \frac{c_{ij}}{n_{j}}$, where $n_i, n_j$ are the row and column totals. The first, $f^{o}$ is the overall fraction, where counts are normalised by the overall number of observations $n$. This can only give an overview and is generally less useful for feature selection. 
