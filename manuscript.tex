...
\Plaintitle{Interactively Visualizing Multivariate Market Segmentation Using pytourr} %% without formatting
...
This paper is organised as follows XXX The methods are illustrated using Austrian and Australian tourism data provided in \cite{leisch2018market}.

\section{Interactive interface for partitioning}
...
While tour animations are best obtained within R using the tourr package~\citep{tourr}, it does not enable the interactivity required for example for a manual tour~\citep{laa2023new}. Interactive graphics are available when using Javascript, as implemented in the detourr package \citep{detourr}. This allows to replay a recorded tour path with interactive graphics, and can also be linked with additional displays, but lacks capabilities for manual tours.
...
\begin{figure}[h!]
    ...
    \caption{Overview of the GUI, consisting of the sidebar (left) and display area (right). A: Sidebar. B: Display area. C: Feature selection checkboxes. D: Subset selection with color indicators. E: Frame selection interface. F: Save and load buttons. G: Interface for starting new tours. H: Metric selection interface.}
...
\section{Workflow with the pytourr package}
...
Ultimately, manual data exploration serves as a crucial complement to automated methods, providing the flexibility to incorporate context, expert judgment, and specific analytical goals. This approach enables analysts to refine initial results and adapt them to the complexities of real-world scenarios, leading to more nuanced interpretations and actionable insights. The pytourr package offers a user-friendly and powerful tool for conducting such analyses, bridging the gap between automated clustering and interactive exploration.

By integrating interactive visualization capabilities, the pytourr package empowers users to dynamically engage with their data, making it possible to uncover subtle patterns and relationships that might otherwise remain hidden. This flexibility is particularly valuable in complex datasets where traditional methods may fall short. Additionally, the package's broad applicability across various data types and analytical contexts highlights its potential as a versatile tool for a wide range of research and practical applications.

Looking forward, expanding the interactive features and supporting additional visualization and data types could further enhance the package’s utility, making it an even more powerful tool for exploratory data analysis. Future developments could also include integrating advanced machine learning techniques, such as enabling users to generate new cluster solutions directly within the GUI. Another promising direction would be the implementation of sliced tours, which would enhance the package's capability to analyze concave and nonlinear structures.
