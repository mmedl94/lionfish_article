...
We sincerely thank you for your valuable suggestions, which we believe have greatly improved the quality of both the article and the lionfish software.
...
\begin{itemize}
    ...
    \textbf{Response:} We have added an additional analysis using data on a Likert scale. Unfortunately, we could not find a suitable dataset with fully continuous variables. However, an analysis of a biotechnological dataset with fully continuous variables is available in the official Lionfish documentation. We now mention in Section 7 that additional use cases can be found in the documentation. The analysis of biotechnological data was not included in the article as it did not fit the chosen theme, which was part of Fritz Leisch's work.    
\end{itemize}
...
\begin{itemize}
    ...
    \textbf{Response:} We have expanded on this statement to clarify its meaning.
...
higher resolution but slower display updates. The "Animate" element allows users to animate tours, automatically shifting to the next projection after a specified amount of time. The "Blend out projection threshold" offers the ability to hide projection axes with a norm smaller than a chosen threshold.
	...
	\textbf{Revised text}: In Figure 7B, we observed that clusters 1 and 3 contained data points that did not fit well into their respective clusters.
